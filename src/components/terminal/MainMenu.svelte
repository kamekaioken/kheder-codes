...
>
	<TerminalOutput class="mt-2.5 mb-1">{session.labels.choose}</TerminalOutput>
	...
	<TerminalMenu label={session.labels.navLabel} class="mt-2 mb-1">
		{#each session.items as item, index (item.id)}
			<TerminalRow
				...
				aria-current={item.href && item.id === session.activeItemId
					? 'page'
					: undefined}
				aria-expanded={item.href === null
					? item.id === session.activeItemId
					: undefined}
				aria-controls={item.href === null ? 'terminal-body' : undefined}
				data-panel={item.href === null ? '' : undefined}
